...
def load_news():
    ...
    try:
        ...
        news_cache = r.json()
        st.success(f"Календарь обновлён: {len(news_cache)} событий", icon="✅")
    except:
        st.warning("Не удалось загрузить новости")
...
def fetch_pair_data(ticker, tf):
    try:
        ...
        if strong_call:
            msg = f"🔥 <b>MAX CALL</b> {pair_name} {tf}\nЦена: {price}\nADX: {adx:.1f} | BB: Lower"
            send_telegram(msg)
            return {"Пара": pair_name, "ТФ": tf, "Цена": price, "RSI": round(rsi,1), "ADX": round(adx,1), "BB": bb_pos, "Сигнал": "✅ CALL", "Сила": "MAX", "Рекомендация": "Открывай CALL"}
        elif strong_put:
            msg = f"🔥 <b>MAX PUT</b> {pair_name} {tf}\nЦена: {price}\nADX: {adx:.1f} | BB: Upper"
            send_telegram(msg)
            return {"Пара": pair_name, "ТФ": tf, "Цена": price, "RSI": round(rsi,1), "ADX": round(adx,1), "BB": bb_pos, "Сигнал": "❌ PUT", "Сила": "MAX", "Рекомендация": "Открывай PUT"}
        else:
...
with st.sidebar:
    ...
    refresh_rate = st.slider("Интервал (секунды)", 30, 180, 45)
...
st.caption("Приложение работает 24/7 на сервере. Сильные сигналы автоматически отправляются в Telegram.")
